...
Do **not** call `create an agent group`, `record the task`, `Agent`, `send follow-up instructions`, or `close the agent group` in dry-run mode.
...
```bash
~/.codex/plugins/ycc/skills/deep-research/scripts/check-prerequisites.sh "$OUTPUT_DIR"
```
...
record the task: subject="historian: Historical evolution of <subject>",              description="Historical evolution, failed attempts, forgotten alternatives."
record the task: subject="contrarian: Disconfirming evidence on <subject>",           description="Expert critiques, documented failures, disconfirming evidence."
record the task: subject="analogist: Cross-domain parallels for <subject>",           description="Biological/military/economic analogies and cross-domain transfers."
record the task: subject="systems-thinker: Second-order effects of <subject>",        description="Stakeholder mapping, causal chains, feedback loops."
record the task: subject="journalist: Current state of <subject>",                    description="Key players, latest developments, present-day dynamics."
record the task: subject="archaeologist: Past solutions for <subject>",               description="Obsolete approaches, 10-50 year old attempts, forgotten lineages."
record the task: subject="futurist: Speculative futures for <subject>",               description="Patents, speculative research, 2030+ predictions."
record the task: subject="negative-space-explorer: What's NOT discussed about <subject>", description="Adoption barriers, silent assumptions, omissions."

# Batch 2 — Phase 2 crucible (blocked by all Batch 1 tasks)
record the task: subject="ach-analyst: Analysis of Competing Hypotheses",             description="Generate 5+ mutually exclusive hypotheses and seek disconfirming evidence."
record the task: subject="contradiction-mapper: Cross-persona disagreements",         description="Map contradictions between the 8 persona findings."
update the task tracker: addBlockedBy=[<historianTaskId>, <contrarianTaskId>, <analogistTaskId>, <systemsThinkerTaskId>, <journalistTaskId>, <archaeologistTaskId>, <futuristTaskId>, <negativeSpaceExplorerTaskId>]
update the task tracker: addBlockedBy=[<historianTaskId>, <contrarianTaskId>, <analogistTaskId>, <systemsThinkerTaskId>, <journalistTaskId>, <archaeologistTaskId>, <futuristTaskId>, <negativeSpaceExplorerTaskId>]

# Batch 3 — Phase 3 strategic (blocked by all Batch 2 tasks)
record the task: subject="tension-mapper: Maximum disagreement points",               description="Identify highest-tension disagreements from crucible output."
update the task tracker: addBlockedBy=[<achAnalystTaskId>, <contradictionMapperTaskId>]
record the task: subject="pattern-recognizer: Unexpected historical echoes",          description="Find surprising pattern matches across persona findings and crucible."
update the task tracker: addBlockedBy=[<achAnalystTaskId>, <contradictionMapperTaskId>]
record the task: subject="negative-space-analyst: Unanswered questions",              description="Document research gaps and what remains unresolved after crucible."
update the task tracker: addBlockedBy=[<achAnalystTaskId>, <contradictionMapperTaskId>]
record the task: subject="innovation-agent: Novel hypotheses from combinations",      description="Synthesize novel hypotheses by recombining persona viewpoints."
update the task tracker: addBlockedBy=[<achAnalystTaskId>, <contradictionMapperTaskId>]
```

Capture the concrete IDs returned by the preceding `record the task` calls (`historianTaskId`, `achAnalystTaskId`, etc.) and substitute them directly into each `addBlockedBy` list before running `update the task tracker`.

The team persists across all three phases; teammates are spawned per phase (Steps 7, 11, 13) and shut down after each phase completes (Steps 8.5, 12.5, 13.5). `close the agent group` runs inside Step 13.5 — after the final shutdown — before Phase 4 synthesis.
...
```bash
cat ~/.codex/plugins/ycc/skills/deep-research/templates/persona-prompts.md
```
...
All 8 `record the task` entries were registered up front in Step 5.5 — do not re-create them here.
...
After spawning, use `the task tracker` to confirm all 8 Batch 1 tasks are `completed` before proceeding to Step 8. Do not rely on agent return values alone — check the shared task state.
...
Do NOT `close the agent group` — the team persists for Phases 2 and 3.
...
```bash
cat ~/.codex/plugins/ycc/skills/deep-research/templates/analysis-prompts.md
```
...
Both `record the task` entries were registered up front in Step 5.5 (blocked by all Batch 1 tasks — they become eligible once Phase 1 completes).
...
After spawning, use `the task tracker` to confirm both Batch 2 tasks are `completed` before proceeding to Step 12.
...
Do NOT `close the agent group` — the team persists for Phase 3.
...
All 4 `record the task` entries were registered up front in Step 5.5 (blocked by all Batch 2 tasks).
...
After spawning, use `the task tracker` to confirm all 4 Batch 3 tasks are `completed` before proceeding to Step 13.5.
...
**ALWAYS** `close the agent group` before Phase 4 — Phase 4 is orchestrator-only synthesis work and the team is no longer needed. Leaving the team live pollutes the workspace across skill invocations.
...
- **Majority failure in a batch** (≥50% failed): Abort the skill. Send `shutdown_request` to any still-active teammates, call `close the agent group`, and report to the user. Suggest retrying without `--team` (standalone sub-agent fallback).
- **Mid-run user abort**: `send follow-up instructions(shutdown)` to every active teammate, then `close the agent group`. Never exit the skill with the team still live.
...
```bash
cat ~/.codex/plugins/ycc/skills/deep-research/templates/report-structure.md
```
...
```bash
~/.codex/plugins/ycc/skills/deep-research/scripts/validate-research.sh "$OUTPUT_DIR"
```
